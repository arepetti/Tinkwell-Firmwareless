...
public class KeysControllerTests : IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task Create_AdminWithDaysValidTooHigh_ShouldReturnCreated()
    {
        // Arrange
        var client = _factory.CreateClient();
        var (adminKey, vendorId) = await CreateAdminKeyAndVendor();
    ...
    [Fact]
    public async Task Create_AdminWithDaysValidTooHigh_ShouldReturnBadRequest()
    {
        // Arrange
        var client = _factory.CreateClient();
        var (key, vendorId, id) = await CreateUserKeyAndVendor();
        client.DefaultRequestHeaders.Add(ApiKeyAuthHandler.HeaderName, key);
        var request = new KeysService.CreateRequest(vendorId, "Valid Name", "User", 366, [Scopes.KeyRead]);
